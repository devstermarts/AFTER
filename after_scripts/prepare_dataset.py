...
flags.DEFINE_string(
    ...
    None,
    help=
    'Folder containing the midi files, if a midi file parser is used',
    required=False)
...
flags.DEFINE_string('waveform_augmentation',
                  default="shift_stretch",
                  help="Perform data augmentation for the timbre input : [none, shift, stretch, shift_stretch]")
...
def main(dummy):
    ...
    if FLAGS.waveform_augmentation == "none":
        ...
        if FLAGS.waveform_augmentation == "shift_stretch":
            waveform_augmentation = PSTS(ts_min = 0.51, ts_max = 1.99, pitch_min = -6, pitch_max = 6, sr = FLAGS.sample_rate, chunk_size = FLAGS.num_signal//4)
        elif FLAGS.waveform_augmentation == "stretch":
            waveform_augmentation = PSTS(ts_min = 0.51, ts_max = 1.99, pitch_min = 0, pitch_max = 0, sr = FLAGS.sample_rate, chunk_size = FLAGS.num_signal//4)
        elif FLAGS.waveform_augmentation == "shift":
            waveform_augmentation = PSTS(ts_min = 1, ts_max = 1, pitch_min = -6, pitch_max = 6, sr = FLAGS.sample_rate, chunk_size = FLAGS.num_signal//4)
        else:
    ...
    if len(FLAGS.descriptors)>0:
        waveform_descriptors = AudioDescriptors(sr=FLAGS.sample_rate, descriptors=FLAGS.descriptors, hop_length = 512, n_fft = 2048)
    ...
    for i, (file, midi_file, metadata) in enumerate(
        ...
        if midi_file is None:
            ...
            if audio.shape[-1] > FLAGS.num_signal and audio.shape[-1]%FLAGS.num_signal > FLAGS.num_signal//2:
                audio = np.pad(
                    audio,
        ...
        for j, chunk in enumerate(chunks):
            ...
            if midi_data is not None:
                silence_test, midi = get_midi(copy.deepcopy(midi_data), chunk_number = chunk_index)
            else:
                midi = None
                silence_test = np.max(
            ...
            if len(chunks_buffer) == FLAGS.batch_size or (
                ...
                if emb_model is not None:
                    ...
                    if waveform_augmentation is not None:
                        ...
                        for i in range(FLAGS.num_augments):
                            augmented_buffers = waveform_pool.map(waveform_augmentation, chunks_buffer)
                            augmented_buffers_torch = [torch.from_numpy(a).reshape(1,1,-1).to(device) for a in augmented_buffers]
                            z_augmented = [emb_model.encode(a).squeeze().cpu().numpy() for a in augmented_buffers_torch]
                            augments["augment_"+FLAGS.waveform_augmentation+"_" + str(i)] = z_augmented
                    else:
                        augments = None
                    ...
                    features = {}
                    if len(FLAGS.descriptors)>0:
                        descriptors_buffers = waveform_pool.map(partial(waveform_descriptors, z_length = z.shape[-1]), chunks_buffer)
                        for k in descriptors_buffers[0]:
                            features[k] = [d[k] for d in descriptors_buffers]
                        
                    # Beat tracking
                    if FLAGS.beat_track:
                        beat_data = [beat_tracker(chunk, z_length = z.shape[-1]) for chunk in chunks_buffer]
                        features["beat_clock"] = [b["beat_clock"] for b in beat_data]
                        features["downbeat_clock"] = [b["downbeat_clock"] for b in beat_data]
                ...
                for k, (array, curz, midi, cur_metadata) in enumerate(zip(
                    ...
                    if augments is not None:
                        for key, augmented_buffers in augments.items():
                            ae.put_array(key, augmented_buffers[k], dtype=np.float32)
